...
client = test(pexpect.spawn, "Client did not start properly 1",
              client_name, [ip, port])

test(client.expect,  "Client did not start properly 2", prompt)
...
for t in commands:
    ...
    if not server.isalive():
        fail("Server crashed after command 1 '{}'".format(com))
...
client = test(pexpect.spawn, "Client did not start properly 3",
...
for t in commands[:breakpoint]:
    test_command(client, com, out)
    if not server.isalive():
        fail("Server crashed after command 2 '{}'".format(com))
    
client.kill(SIGKILL)

# Test connection after a client crashed
client = test(pexpect.spawn, "Client did not start properly 4",
              client_name, [ip, port], echo=False)
...
for t in commands[breakpoint:]:
    ...
    test_command(client, com, out)
    if not server.isalive():
        fail("Server crashed after command 3 '{}'".format(com))
test(client.sendline, "Error while exiting client", "exit")
